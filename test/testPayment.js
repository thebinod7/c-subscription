...
contract('Payment', addresses => {
	...
	it('should NOT create a plan', async () => {
		...
		await expectRevert(payment.createPlan(token.address, 0, THIRTY_DAYS, { from: merchant }), 'amount needs to be > 0');
		await expectRevert(payment.createPlan(token.address, 100, 0, { from: merchant }), 'frequency needs to be > 0');
	});
	...
	it('should NOT create a subscription', async () => {
		await expectRevert(payment.subscribe(0, { from: subscriber }), 'this plan does not exist');
	});
	...
	it('should NOT cancel subscription', async () => {
		await expectRevert(payment.cancel(0, { from: subscriber }), 'this subscription does not exist');
	});
});
